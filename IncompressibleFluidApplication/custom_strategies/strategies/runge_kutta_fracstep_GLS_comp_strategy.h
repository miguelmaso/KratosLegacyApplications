...
*   Date:                $Date: 2009-04-24 14:23:32 $
*   Revision:            $Revision: 1.12 $
*
* ***********************************************************/


#if !defined(KRATOS_RUNGE_KUTTA_GLS_COMP_STRATEGY)
#define  KRATOS_RUNGE_KUTTA_GLS_COMP_STRATEGY
...
template<unsigned int TDim, class TSparseSpace,
         ...
         class TLinearSolver
         >
class RungeKuttaFracStepCompStrategy
...
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION(  RungeKuttaFracStepCompStrategy );
...
//std::cout << "SONO QUI" << std::endl;
        ...
        */
        ...
        if (TDim==2)
        {
            ...
            if (typeid(ref_el) != typeid(el))
                KRATOS_THROW_ERROR(std::logic_error,  "Compressible Runge Kutta Strategy requires utilization of Fluid2DGLS_expl_comp elements " , "");
        }
        ...
        if (TDim==3)
        {
            ...
            if (typeid(ref_el) != typeid(el))
                KRATOS_THROW_ERROR(std::logic_error,  "Compressible Runge Kutta Strategy requires utilization of Fluid3DGLS_expl_comp elements " , "");
        }
    ...
    /** Destructor.
    */
    virtual ~RungeKuttaFracStepCompStrategy() {}
...
//        dens_error=UpdateDensityIdealGas();
    ...
    void ApplySlipBC()
    {
        ...
        for (PointIterator it=mSlipBoundaryList.begin(); it!=mSlipBoundaryList.end(); ++it)
        {
            ...
            if (length==0)
            {
                KRATOS_THROW_ERROR(std::logic_error,  "TO apply SLIP you should calculate normals first! Dont forget to assign Condition2D for that " , "");
            }
            normal*=1.0/length;
    ...
    double UpdateDensityIdealGas()
    {
        KRATOS_TRY
        //A is the constant of the eq. p=A*(rho)^gamma
                ...
                i != BaseType::GetModelPart().NodesEnd() ; ++i)
        {
            ...
            //directly get the density from the equation of state (we have to use the absolute pressure p+100000 here)
            (i)->FastGetSolutionStepValue(DENSITY) = (100000.0+p_new)/(R*t);
        ...
        //and now compute the convergence criretion:
        double error=0.0;
        double aux;
        double dif_dens;
        //I THINK WE HAVE TO SAVE THE OLD DENSITY IN THE DENSITY OLD ITERATION
        for(ModelPart::NodeIterator in = BaseType::GetModelPart().NodesBegin() ;
                in != BaseType::GetModelPart().NodesEnd() ; ++in)
        {
            //reset the mass factors to Zero
            dif_dens=(in->FastGetSolutionStepValue(DENSITY)-in->FastGetSolutionStepValue(DENSITY,1))/(in->FastGetSolutionStepValue(DENSITY,1));
            aux=dif_dens*dif_dens;
            if (aux>error)
                error=aux;
        }
        KRATOS_WATCH(error)
...
private:
    ...
    */
    ...
    /** Copy constructor.
    */
    RungeKuttaFracStepCompStrategy(const RungeKuttaFracStepCompStrategy& Other);
...
#endif /* KRATOS_RUNGE_KUTTA_GLS_COMP_STRATEGY  defined */
